function initActivity(activity){
	...
	jQuery.each(activity.questions, function(key, values){
		...
		if(typeof(values)=="string"){
			...
			drag_drop_questions += '<li class="'+has_single_text+' background_audio"><div class="droppable_label">'+ values.replace(/___/g, '<input readonly type="text" class="droppable_div" /></div><div class="droppable_label">')+'</div></li>';

		} else {
			jQuery.each(values, function(k, v){
				var v = v+"";
				drag_drop_questions += '<li class="drag_drop_multiple background_audio" style="">'+ v.replace('___', ' <input readonly type="text" class="droppable_div" />') +'</li>';
			});
		}
