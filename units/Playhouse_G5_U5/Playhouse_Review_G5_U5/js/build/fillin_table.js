				...
				htmlStmt += '<table>';
					...
					for(let col=0; col<numOfRow; col++){ //4
						...
						for(let row=0; row < numOfColumns; row++){
							...
							// ---------------------------------------------------- //
							// خانة فاضية (spacer) بين عمود2 وعمود3 - فراغ بصري
							// حقيقي بين الصندوقين. مالها class="que" فما
							// FillIn.js رح يلتقطها إطلاقاً، صفرية التأثير
							// على التحقق/الداتا.
							// ---------------------------------------------------- //
							if(row == 2){
								htmlStmt += '<td class="fillin_spacer_td"></td>';
							}
