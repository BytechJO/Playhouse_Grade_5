			...
			for (x = 0; x < numOfQuestions; x++) {
				...
				if (tmpObj.singleword) {
				  ...
				  qStr = words.map((word, index) => {
					if (word !== '') {
					  wordIndex++;
					  return '<div class="txt-audioIcon off d-flex contant min_w_fit_contant" data-audio="' + tmpObj.textaudios[wordIndex] + '">' + word + '</div>'
					}
				...
				if(aObj.numbering != 'none'){
					...
					if (tmpObj.options_words != undefined && tmpObj.options_words != '') {
						...
						for(let y=0; y<tmpObj.options_words.length; y++){
							htmlStmt += '<div class="txt-audioIcon off d-flex contant "' + tmpObj.options_words_audios[y] + '">';
