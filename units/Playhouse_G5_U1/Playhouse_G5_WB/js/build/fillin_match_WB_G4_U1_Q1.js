...
jQuery.each(activity.options, function(key, value){
  html += '<div class="draggable_div" data-value="'+value+'" data-qno="'+key+'" style="background-color: transparent;">'+value+'</div>';
	...
	*/
	...
	html += `<div class="drag_drop_questions"><ul class="${classOption} mt-5" style="justify-content: space-around;">`;
	jQuery.each(activity.questions, function(key, values){

			
		if(typeof(values)=="string"){

			html += `<li class='qline_${(key+1)} ${styleListOption}'>`+ values.replace(/___/g, ` <input readonly type="text" class="droppable_div ${styleInputOption}" />`) +'</li>';
