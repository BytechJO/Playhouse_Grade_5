		...
		for(x= 0;x<numberofCols;x++){	
			...
			for(y= 0;y<numOfQinCol;y++){
				...
				if(typeof tpOb != undefined && tpOb != null ){
                    ...
                    var tmphtml = "";
                    var fillBoxesArr=[];
                    for (var numOfSets = 0; numOfSets < (opts).length; numOfSets++) {
                        fillBoxesArr[numOfSets]='';        

						console.log("length",opts,numOfSets);
						// debugger
						if(tpOb.inputbox == "no"){
							// if (numOfSets == 0){
							// 	tmphtml = "<span class='pick_set' id='pick_set_"+currQueNum+"_1'>";
							// } else if (numOfSets == 1) {
							// 	tmphtml = "<span class='pick_set' id='pick_set_"+currQueNum+"_2'>";
							// }
							tmphtml = "<span class='pick_set' id='pick_set_"+currQueNum+"_"+(numOfSets+1)+"'>";
						}else{
							tmphtml = "<span class='pick_set' id='pick_set_"+currQueNum+"_"+(numOfSets+1)+"'><input class='text_input_area d-none' type='text' readonly disabled>";
						}						
                        var tmpOpt = opts[numOfSets];

                        for(var aa=0;aa<tmpOpt.length;aa++){
                            tmphtml += '<span class="pick" id="pick_'+currQueNum+'_'+(numOfSets+1)+'_'+(aa+1)+'" data-opttext="'+tmpOpt[aa]+'">'+tmpOpt[aa]+'</span>';
						...
						tmphtml += '</span>';
                        fillBoxesArr[numOfSets] = tmphtml;
