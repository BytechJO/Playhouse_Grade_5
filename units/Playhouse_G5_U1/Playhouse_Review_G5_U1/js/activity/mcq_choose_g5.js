//  ****************************************** //
//  MCQ_Choose - Version no: 1.2
//  Date written - August 14, 2020 
//  Date updated - August 23, 2020 (audio enable after validation)
//  Date updated - (support multiple correct answers inside the same pick_set)
//  ****************************************** //
...
MCQ.prototype = {
    ...
    validate:function(){
        ...
        $area.find('.que').each(function(){
            ...
            resultArr[qIndx-1] = 0;
            // ----------------------------------------------------------- //
            // _cAns بتضل متل ما هي (مصفوفة، عنصر واحد لكل pick_set)، بس
            // هلق كل عنصر فيها ممكن يكون رقم واحد (إجابة وحيدة، الوضع القديم)
            // أو مصفوفة أرقام (أكتر من إجابة صحيحة جوا نفس الـ pick_set).
            // ----------------------------------------------------------- //
            var _cAns = fDataObj.answer;
            var tmpRes = [];
            ...
            $(this).find('.pick_set').each(function(){
                ...
                var _setCorrAnsArr = $.isArray(_setCorrAns) ? _setCorrAns : [_setCorrAns];

                // بنجمع كل العناصر المحددة (selected) جوا هاد الـ pick_set،
                // مش بس أول وحدة متل قبل.
                var _selectedArr = [];
                $(this).find('.selected').each(function(){
                    _selectedArr.push( parseInt( ($(this).attr('id')).split('_')[3] ) );
                });

                var _sortedSel  = _selectedArr.slice().sort(function(a,b){ return a-b; });
                var _sortedCorr = _setCorrAnsArr.slice().sort(function(a,b){ return a-b; });

                var isMatch = (_sortedSel.length == _sortedCorr.length) &&
                              _sortedSel.every(function(val, idx){ return val == _sortedCorr[idx]; });

                console.log('pick_set >> ', $(this).attr('id'), _setCorrAnsArr, _selectedArr, isMatch);
                if(isMatch){
                    tmpRes[setIndx-1] = 1;
                }
            });
