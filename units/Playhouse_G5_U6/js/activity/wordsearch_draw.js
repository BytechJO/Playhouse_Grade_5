...
//  Date updated - Aug 2026 (auto-fill "write the antonyms" blank lines on correct find)
//  ****************************************** //
...
WordSearch.prototype = {
                ...
                if(isCorrect){  
                    if(corr.find('.corr_'+tWord).length == 0){
                        ...
                        if(_tAns[0] == tWord && (_i.style.display == 'none')){
                            ...
                            // ------------------------------------------------------------
                            // Auto-write the found word into its matching blank line.
                            // The blank lines are readonly — the student never types here,
                            // the word appears the instant it's found correctly.
                            // ------------------------------------------------------------
                            var $writeLine = $(ob.activity_area).find('#write_'+(theIndx+1));
                            if($writeLine.length > 0){
                                $writeLine.val(ob.wordList[theIndx]);
                                $writeLine.addClass('filled');
                            }
                        }
                        var newObj = {};
                        newObj.word = tWord;
    ...
    buildWordList:function(ob){
        ...
        if(ob.wordList.length > 0){
            for (var r = 0; r < ob.wordList.length; r++) {
                ...
                var shownWord = document.createElement("strong");
                if(ob.css_words != 'none'){
                ...
                divEl.appendChild(t);
                divEl.appendChild(shownWord);
                if (ob.data_obj.questions[0].antonyms[r] != "" && ob.data_obj.questions[0].antonyms[r] != undefined){
                    divEl.getElementsByTagName('strong')[0].innerHTML = ob.data_obj.questions[0].antonyms[r];
    ...
    reset:function(){
        ...
        // clear the auto-filled "write the antonyms" blank lines too
        var $writeLines = $(ob.activity_area).find('.write_line');
        $writeLines.val('');
        $writeLines.removeClass('filled');
    },
