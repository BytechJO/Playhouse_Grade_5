function buildFillInBody(aObj) {	
	...
	if(typeof aObj !=undefined && aObj !=null){		
		...
		// ---- NEW: grouped word box (language columns), matches the
		// reference image. Only renders if aObj.wordGroups is provided -
		// pages that only set the old flat "options" array are completely
		// unaffected and keep rendering exactly as before. ----
		if (typeof aObj.wordGroups != undefined && aObj.wordGroups != null && aObj.wordGroups.length > 0) {
			htmlStmt += '<div class="word_box_grouped">';
			if (aObj.wordBoxTitle != undefined && aObj.wordBoxTitle != null && aObj.wordBoxTitle != '') {
				htmlStmt += '<div class="word_box_title">' + aObj.wordBoxTitle + '</div>';
			}
			htmlStmt += '<div class="word_box_cols d-flex flex-wrap">';
			jQuery.each(aObj.wordGroups, function (gi, grp) {
				htmlStmt += '<div class="word_box_col">';
				htmlStmt += '<div class="word_box_label">' + grp.label + '</div>';
				jQuery.each(grp.words, function (wi, w) {
					htmlStmt += '<div class="word_box_word">' + w + '</div>';
				});
				htmlStmt += '</div>'; // word_box_col
			});
			htmlStmt += '</div>'; // word_box_cols
			htmlStmt += '</div>'; // word_box_grouped
		} else if (typeof aObj.options != undefined && aObj.options != null) {
			// ---- original flat word-options box, unchanged ----
