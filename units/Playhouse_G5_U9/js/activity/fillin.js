...
FillIn.prototype = {
    ...
    for (var i = 0; i < elsQue.length; i++) { 
        ...
        // ============ جديد: تجاهل السؤال بالكامل إذا answer فاضية بالداتا ============
        var _answerIsEmpty = (fDataObj.answer == undefined || fDataObj.answer == null ||
            (typeof fDataObj.answer === 'string' && $.trim(fDataObj.answer) === '') ||
            (Array.isArray(fDataObj.answer) && fDataObj.answer.length === 0));

        if(_answerIsEmpty){
            resultArr[i] = 1; // نعتبره "غير مؤثر" على النتيجة العامة
            continue;         // نتخطى هالسؤال بالكامل
        }
        // ============================================================================
        ...
        // ============ جديد: نجمع بس الاندكسات يلي عندها إجابة فعلية (مش فاضية) ============
        var _validIndices = [];
        for(var vi=0; vi<_cAns.length; vi++){
            if($.trim(_cAns[vi]) !== ''){
                _validIndices.push(vi);
            }
        }
        // ================================================================================

        if((_uAns.length>0) && (_cAns.length == _uAns.length)){
            for(var vv=0; vv<_validIndices.length; vv++){
                var cc = _validIndices[vv]; // نشتغل بس على الخانات يلي إجابتها مش فاضية

                _cAns[cc] = (_case == 'yes')? _cAns[cc]: _cAns[cc].toLowerCase();  
                _cAns[cc] = (_cAns[cc]).replace(/\s/g, '');
                _uAns[cc] = ((_uAns[cc] != undefined)? _uAns[cc] : '').replace(/\s/g, '');
        ...
        // ============ جديد: المقارنة صارت مقابل عدد الخانات الصحيحة الفعلية بس ============
        if(_corr == _validIndices.length && _wrong == 0){
            resultArr[i] = 1;
