//  ****************************************** //
//  MCQ - Version no: 2 (event-delegation fix)
//  Date updated - July 03, 2020
//  ****************************************** //
window.MCQ_01 = function (obj, dataObj) {
...
MCQ_01.prototype = {
  ...
  // ---- FIX: delegate the click from the stable .options container
  // instead of binding one listener per .pick element. The old code
  // did `e.querySelectorAll('.pick')` and attached a listener to each
  // element found AT THAT MOMENT - if the pick elements are appended
  // to the DOM after MCQ is constructed (a build-order/timing issue
  // between the builder and initiateActivityType()), that query
  // returns nothing and no clicks ever get wired up, silently, with
  // no console error. Delegation fixes this permanently regardless of
  // build order, since .options itself already exists when MCQ runs. ----
  listen: function (ob) {
        console.log('MCQ listen() called, activity_area =', ob.activity_area, '| shape =', (ob.data_obj).shape);
    ...
    e.addEventListener("click", function (clickEvt) {
      var pickEl = clickEvt.target.closest(".pick");
      if (!pickEl || !e.contains(pickEl)) {
        return;
      }
      console.log("shape:", selShape, "| MCQ instance init count check");
      ...
      var $pick = $(pickEl);
      var _thisId = $pick.attr("id");
      var _parentNum = _thisId.split("_")[1];
      var thisQue = $("#" + _thisId).closest("#que_" + _parentNum);
      var thisQuePicks = thisQue.find(".pick");
      self.showIcons(false);
      var isSel = !$pick.hasClass("selected");

      if (selectTyp == "single") {
        self.resetAllPicks(thisQue, thisQuePicks);
      } else {
        self.resetAllPicks(thisQue, [$pick]);
      }
      if (isSel) {
        ...
        if (selShape == "cross") {
          ...
          $pick.find("svg").removeClass("fillWhite");
          $pick.find("svg").addClass("fillBlue");
        } else if (selShape == "tickbox") {
          $pick.find(".selectTick").css("display", "block");
        } else {
          if (selectBgColor != "none") {
            $pick.css("background-color", selectBgColor);
          } else {
            $pick.addClass("selectedDefault");
          }
        }
      }
      document
