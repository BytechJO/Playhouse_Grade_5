function buildFillInBody(aObj) {	
	...
	if(typeof aObj !=undefined && aObj !=null){		
		...
		htmlStmt += '<div class="screen_elements d-flex flex-column justify-content-center align-items-center h-100">';
			...
			// ---------------------------------------------------------------
			// WORD BANK (multi-category word list shown ABOVE the questions)
			// aObj.wordBank = [ { title, titleAudio, words:[...], wordsAudios:[...] }, ... ]
			// Renders as N columns, one per category (Latin / French / Indian / Arabic / etc.)
			// ---------------------------------------------------------------
			if(aObj.wordBank != undefined && aObj.wordBank.length > 0){
				htmlStmt += '<div class="word_bank_container d-flex flex-wrap justify-content-center">';
				for(let wb = 0; wb < aObj.wordBank.length; wb++){
					var wbData = aObj.wordBank[wb];
					htmlStmt += '<div class="word_bank_col">';
						htmlStmt += '<div class="audioIcon off contant word_bank_title_audio" data-audio="'+(wbData.titleAudio || '')+'">';
							htmlStmt += '<h3 class="title underline">'+wbData.title+'</h3>';
						htmlStmt += '</div>';
						htmlStmt += '<ul>';
						for(let w = 0; w < wbData.words.length; w++){
							var wAudio = (wbData.wordsAudios && wbData.wordsAudios[w]) ? wbData.wordsAudios[w] : '';
							htmlStmt += '<li>';
								htmlStmt += '<div class="audioIcon off contant word_bank_word_audio" data-audio="'+wAudio+'">';
									htmlStmt += wbData.words[w];
								htmlStmt += '</div>';
							htmlStmt += '</li>';
						}
						htmlStmt += '</ul>';
			...
			// ---------------------------------------------------------------
			// legacy verb / preposition word bank (kept for older pages)
			// ---------------------------------------------------------------
			else if(aObj.verbs != undefined || aObj.Preposition != undefined){
				htmlStmt += '<div class="options_container d-flex flex-wrap">';
					htmlStmt += '<div class="verb">';
					htmlStmt += '<h3 class="title">Verb</h3>';
						htmlStmt += '<ul>';
						for(let v = 0; v < aObj.verbs.length; v++){
							htmlStmt += '<li>'+aObj.verbs[v]+'</li>';
						}
						htmlStmt += '</ul>';
					htmlStmt += '</div>';
					//////////////////////////////////
					htmlStmt += '<div class="Preposition">';
					htmlStmt += '<h3 class="title">Preposition</h3>';
						htmlStmt += '<ul>';
						for(let x = 0; x < aObj.Preposition.length; x++){
							htmlStmt += '<li>'+aObj.Preposition[x]+'</li>';
						}
						htmlStmt += '</ul>';
					htmlStmt += '</div>';
				htmlStmt += '</div>';
		...
		htmlStmt += '<div class="group_elm '+aObj.parentClassName+'">';
			...
			htmlStmt += '<div class="ques d-flex flex-wrap">';
